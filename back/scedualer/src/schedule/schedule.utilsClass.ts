  ...
  async getShiftBeforAndAfter(shift: shift) {
   ...
   isShiftpossible(shift: shift | ShiftDto, schedule: ShiftDto[] | schedule) {
    ...
    if (shiftIndexInSched >= 0) {
      ...
      if (shift.shifttStartHour.getHours()-3 === morningTime) {
        const sameDayShift =
          shift.userId ===
          scheduleShifts[shiftIndexInSched + 2].userId;
        if (sameDayShift) {
      ...
      } else if (shift.shifttStartHour.getHours() >= morningTime+12) {
        //in case night shift
    ...
    for (let i = 0; i < scheduleToFill.length; i++) {
      ...
      if (
        ...
        possibleUsersToAssign.map((shift: shift, index: number) => {
          ...
          //   i >= 1 ? shift.userId !== scheduleToFill[i - 1].userId : true;
          // const shiftAfter: boolean =
          //   i < scheduleToFill.length - 1
          //     ? shift.userId !== scheduleToFill[i + 1].userId
          //     : true;

          console.log(
            i > 1 ? scheduleToFill[i - 1]?.userId : scheduleToFill[i],
            i,
          );
          if (ispos === true) {
          ...
          if (selctedShift) {
            const assessNextShift = () => {
              if (i < scheduleToFill.length - 1) {
                ...
                // console.log(
                //   'asses next, after filter - arr length  ',
                //   filterdArr.length,
                // );
  ...
  async twoShiftsADay(
    ...
    if (schedule[0].typeOfShift !== 'long') {
      ...
      const morningAndNight: boolean =
        ...
        nightShift &&
        noonShift &&
        morningShift.shiftType !== 4 &&
        noonShift.shiftType !== 4 &&
        nightShift.shiftType !== 4;
      console.log(
        { morningShift },
        { noonShift },
        { nightShift },
